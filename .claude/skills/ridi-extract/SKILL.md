---
name: ridi-extract
description: Save a Ridi (리디북스, ridibooks.com) or Kakao Page (카카오페이지, page.kakao.com) episode that the user already has open and can read in their own Chrome, as clean Markdown for offline reading, or save a work's public listing (title, author, synopsis, episode list). Works through Claude in Chrome on the user's logged-in session. Probes how the page renders its text (DOM, iframe, shadow root, CSS pseudo-elements, or images/canvas) before extracting, and stops when the text is not there as text. Use it whenever the user gives a ridibooks.com/books or page.kakao.com/content URL and wants an episode or the work's info saved, exported, read offline, or translated. Prefer this over a plain innerText grab, which picks up menus and hidden text and misses pseudo-element runs.
---
